...
В течении 4х недель старый посетитель не будет видеть измений, тогда как новые посетители будут видеть изменения.

Кэшем управлят расширение infrajs/nostore
В Конфиге указываются даты плановых обновлений сайта.
Перед 8 марта меняется статический баннер по этому указана дата 05.03 и у посетителя который зайдёт 1 марта на сайт кэш установится не на 4 недели, а только на 4 дня, до 5 марта.
...
Указано next friday значит, что кэш всегда устанавливается до слеюущей пятницы и каждую пятницу планируется изменение сайта, вместо 2х недель по умолчанию.

Динамикой считается страница сайта. php файлы по умолчанию считаются динамикой, но программист в каждом отдельном случае может изменить это поведение. Например imager/index.php скрипт меняющий картинки ведёт себя также как и статика.

В обычном режиме все добавления, которые делает администратор сайта относится к динамическим данным и применяются в течении 24 часов. Действия администрора по замене одного файла на другой попадает под статические изменения и такие изменения являются статическими, другими словами путь до файлов с данными должен меняться, тогда это динамические изменения.
